...
// (35 entries, valid categories, unique indices) and profile.json.
...
void main() {
  ...
  test('apps.json parses to 35 projects with unique indices 1..35', () async {
    final models = await ProjectsLocalDataSourceImpl(rootBundle).getProjects();
    expect(models, hasLength(35));

    final entities = models.map((m) => m.toEntity()).toList();
    expect(entities.map((e) => e.index).toSet(), {
      for (var i = 1; i <= 35; i++) i,
    });
